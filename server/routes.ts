...
export async function registerRoutes(app: Express): Promise<Server> {
    ...
    try {
      ...
      for (const file of files) {
        ...
        const dbImageData = {
          ...
          studyType: imageData.studyType || imageData.imageType || 'Medical Image',
  ...
  app.put("/api/medical-images/:id/replace", authMiddleware, uploadMedicalImages.single('file'), async (req: TenantRequest, res) => {
    try {
      const imageId = parseInt(req.params.id);
      if (isNaN(imageId)) {
        return res.status(400).json({ error: "Invalid image ID" });
      ...
      // Get the existing medical image from database
